...
x2<-x1 %>%
  ...
  {if (top_up_initial_stock==TRUE) {
    pivot_wider(.,names_from=type,values_from=value) %>%
      mutate(Referrals=case_when(months_waited_id==0 & Reneges<0 ~ Referrals+abs(Reneges))) %>%
...
png(paste0(outdir,"fig3.png"),height=3.5,width=9,units="in",res=400)
print(fig3)
dev.off()
...
btest<-function(backtest_duration) {
  ...
  for (p in bt_scen_periods) {
    ...
    res_p$incompletes<-res_p$node_inflow-res_p$tment-res_p$reneges
...
png(paste0(outdir,"fig4.png"),height=4,width=9,units="in",res=400)
print(fig4)
dev.off()
...
png(paste0(outdir,"fig5.png"),height=2.5,width=5,units="in",res=400)
print(fig5)
dev.off()
...
png(paste0(outdir,"fig6.png"),height=7,width=8,units="in",res=400)
print(fig6)
dev.off()
...
png(paste0(outdir,"fig7.png"),height=7,width=4.5,units="in",res=400)
print(fig7)
dev.off()
